...
class MainAdapter(var products:List<Product>, var clickListener : View.OnClickListener) : RecyclerView.Adapter<MainAdapter.PhotoHolder>() {
    ...
    override fun onBindViewHolder(holder: PhotoHolder, position: Int) {
        ...
        holder?.name?.text = photo.product_name    //  Get tags text field and set its text to our tags from the photo object
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        //  Commented out as the original design of Miami Auto did not have the price on the MainActivity screen
        //
        ...
        if (photo.image.isNotEmpty()) {
            Glide.with(holder?.name?.context)
                .load("https://leedavidsoncontentmanagementsystem1.com/miami_auto/core/bridge_line_1/inventory_images/" + photo.image)
